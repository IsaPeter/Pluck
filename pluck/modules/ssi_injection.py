...
from pluck.generators.ssi_generator import SSIPayloadGenerator2 
from pluck.module import ActiveModule


class SSIInjectionTester(BaseModule):
    ...
    def send_requests(self, request_list):
        ...
        for payload, point, key, req in request_list:
            ...
            if self.analyze_response(response, payload):
                print(f"[!] SSI Injection Found in Point: {point} Parameter: {key}, Payload: {payload}. Request ID: {req.request_id}")
                break;
...
class SSIInjectionTester2(ActiveModule):
    def __init__(self, request):
        super().__init__(request)
        self.name = "SSI Injection Tester"
        self.test_parameters = []
        self.injection_points = ["query", "body", "headers", "cookies"]
        self.excluded_parameters = []
        self.generator = SSIPayloadGenerator2()
